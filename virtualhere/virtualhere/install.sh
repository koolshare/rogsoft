...
install_now(){
	# default value
	local TITLE="virtualhere"
	local DESCR="usb over ip"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/virtualhere.sh" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		sh /koolshare/scripts/virtualhere.sh stop >/dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*virtualhere*" | xargs rm -rf >/dev/null 2>&1
	find /koolshare/init.d -name "*virtualhere*" | xargs rm -rf >/dev/null 2>&1
	...
	# Permissions
	chmod 755 /koolshare/bin/vhusbdarm64 >/dev/null 2>&1
	...
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/virtualhere.sh" ];then
		...
		sh /koolshare/scripts/virtualhere.sh start
	fi
