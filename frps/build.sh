...
do_build() {
	...
	# shrink package: keep only one binary and name it "frps"
	rm -f "${DIR}/build/${MODULE}/bin/"*
	case "${PLATFORM}" in
		hnd|qca|ipq32)
			cp -f "${DIR}/${MODULE}/bin/frps-arm" "${DIR}/build/${MODULE}/bin/frps"
			;;
		mtk|ipq64)
			cp -f "${DIR}/${MODULE}/bin/frps-arm64" "${DIR}/build/${MODULE}/bin/frps"
			;;
		*)
			cp -f "${DIR}/${MODULE}/bin/frps-arm" "${DIR}/build/${MODULE}/bin/frps"
			;;
	esac
	chmod 755 "${DIR}/build/${MODULE}/bin/frps"
