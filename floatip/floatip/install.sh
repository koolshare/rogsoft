...
install_now(){
	...
	cd /tmp
	local ARCH=$(uname -m)
	cp -rf /tmp/${module}/bin/floatip_bin /koolshare/bin/floatip_bin
	cp -rf /tmp/${module}/bin/ipcalc.sh /koolshare/bin/ipcalc.sh
	...
	# re-enable
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/floatip_config.sh" ];then
		echo_date "安装完毕，重新启用${TITLE}插件！"
		sh /koolshare/scripts/floatip_config.sh start
	fi
