...
pushplus_info_text=/tmp/.pushplus_info.md
...
task)
	...
	echo "###### 本次信息推送：定时任务." > ${pushplus_info_text}
	;;
*)
	[ "${pushplus_info_logger}" == "1" ] && logger "[pushplus]: 启动手动推送任务！"
	echo "###### 本次信息推送：手动推送." > ${pushplus_info_text}
	;;
esac
...
send_title=`echo "$pushplus_config_name"| base64_decode`
# 系统运行状态
if [ "${pushplus_info_system}" == "1" ]; then
	...
	echo "#### **系统运行状态:**" >> ${pushplus_info_text}
	echo "##### 系统名称: ${router_name}" >> ${pushplus_info_text}
	echo "##### 系统当前时间: ${router_time}" >> ${pushplus_info_text}
	echo "##### 固件版本: ${router_firmware}" >> ${pushplus_info_text}
	echo "##### 系统模式: ${router_mode}" >> ${pushplus_info_text}
	echo "##### 系统运行时间: ${router_uptime}" >> ${pushplus_info_text}
	echo "##### 平均负载(1,5,15min): ${router_cpu_loadavg_1min}, ${router_cpu_loadavg_5min}, ${router_cpu_loadavg_15min}" >> ${pushplus_info_text}
	echo "##### 总内存: ${router_memtotal}MB   空闲数: ${router_memfree}MB" >> ${pushplus_info_text}
	echo "##### 虚拟内存: `awk 'BEGIN{printf ("%.2f\n",'${router_swaptotal}'-'${router_swapfree}')}'` / ${router_swaptotal} MB" >> ${pushplus_info_text}
	echo "##### JFFS: ${router_jffs_used} / ${router_jffs_total} MB" >> ${pushplus_info_text}
fi
# 温度传感器
if [[ "${pushplus_info_temp}" == "1" ]]; then
	...
	*)
	...
	echo "#### **温度传感器状况:**" >> ${pushplus_info_text}
	echo "##### ${router_temperature}" >> ${pushplus_info_text}
fi
# 网络运行状态
if [[ "${pushplus_info_wan}" == "1" ]]; then
	...
	echo "#### **网络状态信息:**" >> ${pushplus_info_text}
	echo "##### **WAN0状态信息:**" >> ${pushplus_info_text}
	echo "##### 联机类型: ${router_wan0_proto}" >> ${pushplus_info_text}
	echo "##### 公网IPv4地址: ${router_wan0_ip4}" >> ${pushplus_info_text}
	echo "##### 公网IPv6地址: ${router_wan0_ip6}" >> ${pushplus_info_text}
	echo "##### WAN口IPv4地址: ${router_wan0_ip}" >> ${pushplus_info_text}
	echo "##### WAN口DNS地址: ${router_wan0_dns1} ${router_wan0_dns2}" >> ${pushplus_info_text}
	echo "##### WAN口接收流量: ${router_wan0_rx}" >> ${pushplus_info_text}
	echo "##### WAN口发送流量: ${router_wan0_tx}" >> ${pushplus_info_text}
	echo "---" >> ${pushplus_info_text}
	...
	if [ -n "${router_wan1_ifname}" ] && [ -n "${router_wan1_gw}" ]; then
		...
		echo "##### **WAN1状态信息:**" >> ${pushplus_info_text}
		echo "##### 联机类型: ${router_wan1_proto}" >> ${pushplus_info_text}
		echo "##### 公网IPv4地址: ${router_wan1_ip4}" >> ${pushplus_info_text}
		echo "##### 公网IPv6地址: ${router_wan1_ip6}" >> ${pushplus_info_text}
		echo "##### WAN口IPv4地址: ${router_wan1_ip}" >> ${pushplus_info_text}
		echo "##### WAN口DNS地址: ${router_wan1_dns1} ${router_wan1_dns2}" >> ${pushplus_info_text}
		echo "##### WAN口接收流量: ${router_wan1_rx}" >> ${pushplus_info_text}
		echo "##### WAN口发送流量: ${router_wan1_tx}" >> ${pushplus_info_text}
		echo "---" >> ${pushplus_info_text}
	fi
	# ddns状态
	ddns_enable=`nvram get ddns_enable_x`
	if [[ "${pushplus_info_wan}" == "1" ]]; then
		echo "##### **DDNS状态信息:**" >> ${pushplus_info_text}
		echo "##### DDNS域名: $(nvram get ddns_hostname_x)" >> ${pushplus_info_text}
		echo "---" >> ${pushplus_info_text}
	fi
	# ss状态
	...
	get_china_status1(){
		...
		if [ "$?" == "0" ]; then
			echo '##### 国内链接 【'${net_check_time}'】 √' >> ${pushplus_info_text}
		else
			echo '##### 国内链接 【'${net_check_time}'】 ×' >> ${pushplus_info_text}
		fi
	}
	...
	get_foreign_status1(){
		...
		if [ "$?" == "0" ]; then
			echo '##### 国外链接 【'${net_check_time}'】 √' >> ${pushplus_info_text}
		else
			echo '##### 国外链接 【'${net_check_time}'】 ×' >> ${pushplus_info_text}
		fi
	}
	...
	get_china_status(){
		...
		if [ -n "$S1" ]; then
			...
			echo '##### 国内链接 【'${net_check_time}'】 ✓%26nbsp;%26nbsp;'$S2'' >> ${pushplus_info_text}
		else
			echo '##### 国内链接 【'${net_check_time}'】 X' >> ${pushplus_info_text}
		fi
	}
	...
	get_foreign_status(){
		...
		if [ -n "$S1" ]; then
			local S2=`echo $ret|sed 's/time=//g'|awk '{printf "%.0f ms\n",$(NF -3)}'`
			echo '##### 国外链接 【'${net_check_time}'】 ✓%26nbsp;%26nbsp;'$S2'' >> ${pushplus_info_text}
		else
			echo '##### 国外链接 【'${net_check_time}'】 X' >> ${pushplus_info_text}
		fi
	}
	...
	wl_list_tab="|:-|:-|"
	echo "${wl_list_title}" >> ${pushplus_info_text}
	echo "${wl_list_tab}" >> ${pushplus_info_text}
	if [[ "${router_smart_connect}" == "1" ]]; then
		router_ssid=`nvram get wl_ssid`
		[ -n "${router_ssid}" ] && echo "|SmartConnect%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_ssid}|" >> ${pushplus_info_text}
	else
		if [[ "`nvram get wl0_bss_enabled`" == "1" ]]; then
			router_wl0_ssid=`nvram get wl0_ssid`
			[ -n "${router_wl0_ssid}" ] && echo "|2.4G%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl0_ssid}|" >> ${pushplus_info_text}
		fi
		if [[ "`nvram get wl1_bss_enabled`" == "1" ]]; then
			router_wl1_ssid=`nvram get wl1_ssid`
			[ -n "${router_wl0_ssid}" ] && echo "|5G-1%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl1_ssid}|" >> ${pushplus_info_text}
		fi
		if [[ "`nvram get wl2_bss_enabled`" == "1" ]]; then
			router_wl2_ssid=`nvram get wl2_ssid`
			[ -n "${router_wl0_ssid}" ] && echo "|5G-2%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl2_ssid}|" >> ${pushplus_info_text}
		fi
	...
	wlg_list_tab="|:-|:-|"
	echo "${wlg_list_title}" >> ${pushplus_info_text}
	echo "${wlg_list_tab}" >> ${pushplus_info_text}
	...
	do
		if [[ "`nvram get wl${wlg_status}_bss_enabled`" == "1" ]]; then
			...
			echo "|${wlg_name}%26nbsp;%26nbsp;%26nbsp;%26nbsp;|`nvram get wl${wlg_status}_ssid`|" >> ${pushplus_info_text}
			wlg_en="true"
		fi
	done
	if [[ "${wlg_en}" != "true" ]]; then
		echo "|无访客网络||" >> ${pushplus_info_text}
	fi
	#echo "##### SSID: ${router_ssid}" >> ${pushplus_info_text}
fi
# USB设备列表
if [[ "${pushplus_info_usb}" == "1" ]]; then
	echo "---" >> ${pushplus_info_text}
	echo "#### **USB设备列表:**" >> ${pushplus_info_text}
	...
	if [ -n "${usb1_fs_path}" ];then
		...
		echo "##### USB1名称: ${usb1_product}(${usb1_status})" >> ${pushplus_info_text}
		echo "##### USB1容量: ${usb1_used} / ${usb1_total}" >> ${pushplus_info_text}
		echo "##### USB1可用容量：${usb1_available}" >> ${pushplus_info_text}
	else
		echo "##### USB1接口下无设备" >> ${pushplus_info_text}
	fi
	echo "---" >> ${pushplus_info_text}
	...
	if [ -n "${usb2_fs_path}" ];then
		...
		echo "##### USB2名称: ${usb2_product}(${usb2_status})" >> ${pushplus_info_text}
		echo "##### USB2容量: ${usb2_used} / ${usb2_total}" >> ${pushplus_info_text}
		echo "##### USB2可用容量：${usb2_available}" >> ${pushplus_info_text}
	else
		echo "##### USB2接口下无设备" >> ${pushplus_info_text}
	fi
fi
# 客户端列表
if [[ "${pushplus_info_lan}" == "1" ]]; then
	echo "---" >> ${pushplus_info_text}
	echo "#### **客户端列表:**" >> ${pushplus_info_text}
	...
	if [ -n "${arp_list}" ];then
		...
		echo "${arp_list_title}" >> ${pushplus_info_text}
		echo "${arp_list_tab}" >> ${pushplus_info_text}
		arp_num=0
		...
		do
			...
			if [[ "${arp_dhcp_white_name}" == "" ]]; then
				...
				if [[ "${arp_custom_clientlist}" == "" ]]; then
					if [ "$pushplus_info_lan_macoff" == "1" ];then
						echo ${line} | awk '{print "|"$2"%26nbsp;%26nbsp;|"$1"|"}' >>${pushplus_info_text}
					else
						echo ${line} | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"$1"|"}' >>${pushplus_info_text}
					fi
				else
					arp_lease_name=`echo ${arp_custom_clientlist} | awk -F "##" '{print $1}'`
					if [ "$pushplus_info_lan_macoff" == "1" ];then
						echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"}')"${arp_lease_name}|" >>${pushplus_info_text}
					else
						echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"}')"${arp_lease_name}|" >>${pushplus_info_text}
					fi
				fi
			else
				if [ "$pushplus_info_lan_macoff" == "1" ];then
					echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"}')"${arp_dhcp_white_name}|" >>${pushplus_info_text}
				else
					echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"}')"${arp_dhcp_white_name}|" >>${pushplus_info_text}
				fi
			fi
			let arp_num=arp_num+1
		done
	else
		echo "##### 路由器下没有找到在线客户端" >> ${pushplus_info_text}
	fi
fi
# DHCP租期列表
if [[ "${pushplus_info_dhcp}" == "1" ]]; then
	...
	echo "---" >> ${pushplus_info_text}
	echo "##### 现在租约期内的客户端共有 ${total_lease_client} 个,情况如下：" >> ${pushplus_info_text}
	...
	echo "${total_lease_info_title}" >> ${pushplus_info_text}
	echo "${total_lease_info_tab}" >> ${pushplus_info_text}
	...
	do
		...
		if [[ "${trigger_dhcp_white_name}" == "" ]]; then
			...
			if [[ "${dhcp_custom_clientlist}" == "" ]]; then
				if [ "$pushplus_info_dhcp_macoff" == "1" ];then
					echo ${line} | awk '{print "|"$3"%26nbsp;%26nbsp;|"$4"|"}' >>${pushplus_info_text}
				else
					echo ${line} | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"$4"|"}' >>${pushplus_info_text}
				fi
			else
				dhcp_lease_name=`echo ${dhcp_custom_clientlist} | awk -F "##" '{print $1}'`
				if [ "$pushplus_info_dhcp_macoff" == "1" ];then
					echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"}')"${dhcp_lease_name}|" >>${pushplus_info_text}
				else
					echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"}')"${dhcp_lease_name}|" >>${pushplus_info_text}
				fi
			fi
		else
			if [ "$pushplus_info_dhcp_macoff" == "1" ];then
				echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"}')"${trigger_dhcp_white_name}|" >>${pushplus_info_text}
			else
				echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"}')"${trigger_dhcp_white_name}|" >>${pushplus_info_text}
			fi
		fi
		let num=num+1
	done
...
# 软件中心状态
if [[ "${pushplus_info_softcenter}" == "1" ]]; then
	# 软件中心
	echo "---" >> ${pushplus_info_text}
	echo "#### **软件中心状态:**" >> ${pushplus_info_text}
	...
	echo "##### 检测到您共安装了 ${soft_lists_nu} 个插件" >> ${pushplus_info_text}
	rm -rf ${app_file}
	wget --no-check-certificate --quiet ${softcenter_app_url} -O ${app_file}
	if [ "$?" != "0" ];then
		echo "本次检测软件中心更新状态失败。" >> ${pushplus_info_text}
	else
		update=0
		...
		if [ "${COMP_SOFT}" == "1" ];then
			echo "##### 软件中心有新版本了，最新版本: ${softcenter_online_version}" >> ${pushplus_info_text}
			let update+=1
		fi
		...
		do
			...
			do
				...
				if [ -n "$soft_match" ];then
					...
					if [ "${COMP_APP}" == "1" ];then
						echo "##### 插件 ${app} 有新版本了，最新版本: ${app_oneline_version}" >> ${pushplus_info_text}
						let update+=1
					fi
				else
					continue
		...
		#show update message	
		if [ "${update}" == "0" ];then
			echo "##### 检测到所有插件均为最新版本！" >> ${pushplus_info_text}
		else
			echo "##### 快登录路由器进入软件中心更新吧！" >> ${pushplus_info_text}
		fi
	fi
fi
pushplus_send_title="${send_title} 路由器状态信息"
pushplus_send_content=`cat ${pushplus_info_text}`
...
do
	...
	if [ -n $(echo $result | grep "success") ];then
		[ "${pushplus_info_logger}" == "1" ] && logger "[pushplus]: 路由器状态信息推送到 SCKEY No.${nu} 成功！"
	else
		[ "${pushplus_info_logger}" == "1" ] && logger "[pushplus]: 路由器状态信息推送到 SCKEY No.${nu} 失败，请检查网络及配置！"
	fi
done
