...
install_now(){
	# default value
	local TITLE="上海电信IPTV"
	local DESCR="上海电信IPTV，4K IPTV 一键开启"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	if [ "$(dbus get shiptv_dhcp)" == "1" -o "$(dbus get shiptv_vlan)" == "1" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		sh /koolshare/scripts/shiptv_config.sh stop 2>/dev/null
	fi
	...
	if [ ! -L "/koolshare/init.d/S95${module}.sh" ];then
		ln -sf /koolshare/scripts/${module}_config.sh /koolshare/init.d/S95${module}.sh
	fi
	...
	if [ "$(dbus get shiptv_dhcp)" == "1" -o "$(dbus get shiptv_vlan)" == "1" ];then
		...
		sh /koolshare/scripts/shiptv_config.sh start
	fi
