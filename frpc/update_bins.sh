...
fetch_one() {
	...
	src="${TMPDIR}/frp_${version}_linux_${arch}/frpc"
	if [ ! -f "${src}" ]; then
		echo "missing frpc in tarball: ${src}" >&2
		exit 1
	fi
...
fetch_one arm frpc-arm
fetch_one arm64 frpc-arm64

ls -lh "${BIN_DIR}/frpc-arm" "${BIN_DIR}/frpc-arm64"
