...
if [ "${routerhook_info_system}" == "1" ]; then
	...
	echo '"routerMEM":{"all":"${router_memtotal}MB", "free":"${router_memfree}MB"},' >> ${routerhook_info_text}
	echo '"routerSWAP":`awk BEGIN{printf("%.2f\n",${router_swaptotal}-${router_swapfree})}` / ${router_swaptotal},' >> ${routerhook_info_text}
	echo '"routerJFFS":${router_jffs_used} / ${router_jffs_total}' >> ${routerhook_info_text}
	echo '},' >> ${routerhook_info_text}
fi
...
if [[ "${routerhook_info_temp}" == "1" ]]; then
	...
	echo '"tempINFO":{' >> ${routerhook_info_text}
	case "$model" in
	GT-AC5300|GT-AX11000)
		...
		echo '"CPU":"${router_cpu_temperature}°C",' >> ${routerhook_info_text}
...
if [[ "${routerhook_info_wan}" == "1" ]]; then
	...
	echo '"pubIPv4":"${router_wan0_public_ip}",' >> ${routerhook_info_text}
	...
	if [ -n "${router_wan1_ifname}" ] && [ -n "${router_wan1_gw}" ]; then
		...
		echo '"pubIPv4":"${router_wan1_public_ip}",' >> ${routerhook_info_text}
	...
	get_china_status(){
		...
		if [ -n "$S1" ]; then
			...
			echo '"chinaSTATUS":"国内链接 【${net_check_time}】 ✓%26nbsp;%26nbsp;$S2",' >> ${routerhook_info_text}
		else
	...
	get_foreign_status(){
		...
		if [ -n "$S1" ]; then
			local S2=`echo $ret|sed 's/time=//g'|awk '{printf "%.0f ms\n",$(NF -3)}'`
			echo '"foreignSTATUS":"国外链接 【${net_check_time}】 ✓%26nbsp;%26nbsp;$S2",' >> ${routerhook_info_text}
		else
...
if [[ "${routerhook_info_usb}" == "1" ]]; then
	...
	if [ -n "${usb1_fs_path}" ];then
		...
		echo '"used":${usb1_used} / ${usb1_total},' >> ${routerhook_info_text}
	...
	if [ -n "${usb2_fs_path}" ];then
		...
		echo '"used":${usb2_used} / ${usb2_total},' >> ${routerhook_info_text}
...
routerhook_send_content=`cat ${routerhook_info_text}`
