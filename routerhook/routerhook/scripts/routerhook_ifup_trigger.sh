...
echo '"pubIPv4": "${router_wan0_public_ip}",' >> ${routerhook_ifup_text}
...
if [ -n "${router_wan1_ifname}" ] && [ -n "${router_wan1_gw}" ]; then
    ...
    echo '"pubIPv4": "${router_wan1_public_ip}",' >> ${routerhook_ifup_text}
...
routerhook_send_content=`cat ${routerhook_ifup_text}`
