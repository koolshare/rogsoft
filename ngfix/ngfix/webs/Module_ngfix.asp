...
function nvram_erase(){
	if(!confirm('注意：\n\n点击确定将会运行 nvram erase 命令！\n\n')){
		return false;
	}else{
		var id = parseInt(Math.random() * 100000000);
		var postData = {"id": id, "method": "ngfix_nverase", "params": '1', "fields": {}};
		$.ajax({
			type: "POST",
			url: "/_api/",
			data: JSON.stringify(postData),
...
function ofw_guide() {
	...
	note += "<br />"
	note += "<font color='#676767'>&nbsp;&nbsp;&nbsp;&nbsp;如果成功进行了nvram erase后，但不上传Merlin to OFW固件，那么路由器重启后，所有配置将会丢失，路由器需要重新配置。点击取消按钮将不会进行任何操作！</font>"
	note += "<br />"
