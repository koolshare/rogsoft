...
do_build() {
	...
	# shrink package: keep only one binary and name it "natmap"
	rm -f "${DIR}/build/${MODULE}/bin/"*
	case "${PLATFORM}" in
		hnd|qca|ipq32)
			cp -f "${DIR}/${MODULE}/bin/natmap-arm" "${DIR}/build/${MODULE}/bin/natmap"
			;;
		mtk|ipq64)
			cp -f "${DIR}/${MODULE}/bin/natmap-arm64" "${DIR}/build/${MODULE}/bin/natmap"
			;;
		*)
			cp -f "${DIR}/${MODULE}/bin/natmap-arm" "${DIR}/build/${MODULE}/bin/natmap"
			;;
	esac
	chmod 755 "${DIR}/build/${MODULE}/bin/natmap"
