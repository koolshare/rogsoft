...
install_now() {
	TITLE="FakeHTTP 伪装"
	DESCR="将 TCP 连接伪装为 HTTP/HTTPS 协议（NFQUEUE），用于网络流量混淆"
	PLVER="$(cat "${DIR}/version" 2>/dev/null)"

	if [ -f "/koolshare/scripts/fakehttp_config.sh" ]; then
		echo_date "安装前先停止 FakeHTTP，以保证更新成功..."
	...
	chmod 755 /koolshare/bin/fakehttp >/dev/null 2>&1
	...
	if [ "$(dbus get ${module}_enable 2>/dev/null)" = "1" ]; then
		echo_date "检测到已启用，尝试启动 FakeHTTP..."
		sh /koolshare/scripts/fakehttp_config.sh start >/dev/null 2>&1
	fi
}
